...
| 所属模块 | 系统管理 / 用户管理 |
| API | 校验手机号可用性 · GET `/api/v1/system/users/mobile-available` |
| 文档版本 | v0.1 |
| 更新日期 | 2026-09-04 |
...
- **用途与调用场景**：新增 / 编辑用户表单中手机号字段失焦时，前端先校验 11 位手机号格式，格式通过后调用本接口做唯一性异步校验（编辑时排除当前用户，已确认，PRD §8.3、§8.5）。
...
```
GET /api/v1/system/users/mobile-available?mobile=13800008006&excludeUserId=<uuid>
```
...
| --- | --- | --- | --- | --- | --- | --- |
| `mobile` | String | Query | 是 | — | 非空，11 位数字 | 待校验手机号 |
| `excludeUserId` | UUID | Query | 否 | — | UUID 格式 | 排除的用户；编辑时传当前用户（已确认，PRD §8.3） |
...
→ 参数校验
→ 租户内唯一性查询（总览 §5.5）；提供 excludeUserId 时排除该用户
→ 返回 200 + available
```
...
| --- | --- | --- | --- |
| SELECT 唯一性探测 | 总览 §5.5：手机号租户内唯一 | `mobile` 参数 + 租户 | 无 |
...
**`AvailabilityCheckVO`**（设计建议，与用户名可用性接口共用）
...
| --- | --- | --- | --- |
| `mobile` 缺失 / 非 11 位数字 | `USER_REQUEST_INVALID` | 400 | 是 |
...
- [ ] GET `/api/v1/system/users/mobile-available` 可正常调用，成功返回 200。
- [ ] 手机号已被其他用户使用时 `available=false`。
- [ ] 传 `excludeUserId` 时，该用户本人占用的手机号返回 `available=true`（编辑场景）。
